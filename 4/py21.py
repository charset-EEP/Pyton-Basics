...
for i in range(5):
    j=i+1
    print("Veículo "+str(j))
    nomeCarro.append(input(print("Nome: ")))
    kmCarro.append(float(input(print("Km por litro: "))))
    if kmCarro[i]>menor:
        menor=kmCarro[i]
        nomeMenor=nomeCarro[i]
print("Relatório Final")
for i in range(5):
    distancia=1000/kmCarro[i]
    gas=2.25*distancia
    j=i+1
    print(" "+str(j)+" - "+str(nomeCarro[i])+"    -  "+str(kmCarro[i])+" - "+str(distancia)+" litros - R$ "+str(gas))
    print("Menor consumo é o "+str(nomeMenor))
